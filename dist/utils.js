"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var stellar_sdk_1 = require("stellar-sdk");
// export const AstroDollar = new Asset(
//   'AstroDollar',
//   'GC2BKLYOOYPDEFJKLKY6FNNRQMGFLVHJKQRGNSSRRGSMPGF32LHCQVGF'
// )
exports.asset = stellar_sdk_1.Asset.native();
//critical data 
var secretKey = "SDFMDZI3GG3SJ3RNY4IA7D5NQAKB52Z5L6FZNMLHP6FK34IKWMF2P7OF";
function createAccountInLedger(newAccount) {
    return __awaiter(this, void 0, void 0, function () {
        var stellarServer, provisionerKeyPair, provisioner, transaction, result, e_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 3, , 4]);
                    stellar_sdk_1.Network.useTestNetwork();
                    stellarServer = new stellar_sdk_1.Server('https://horizon-testnet.stellar.org');
                    provisionerKeyPair = stellar_sdk_1.Keypair.fromSecret(secretKey);
                    console.log(provisionerKeyPair.publicKey());
                    return [4 /*yield*/, stellarServer.loadAccount(provisionerKeyPair.publicKey())];
                case 1:
                    provisioner = _a.sent();
                    console.log('creating account in ledger', newAccount);
                    transaction = new stellar_sdk_1.TransactionBuilder(provisioner)
                        .addOperation(stellar_sdk_1.Operation.createAccount({
                        destination: newAccount,
                        startingBalance: '2'
                    })).build();
                    transaction.sign(provisionerKeyPair);
                    return [4 /*yield*/, stellarServer.submitTransaction(transaction)];
                case 2:
                    result = _a.sent();
                    console.log('Account created: ', result);
                    return [3 /*break*/, 4];
                case 3:
                    e_1 = _a.sent();
                    console.log('Stellar account not created.', e_1.message);
                    return [3 /*break*/, 4];
                case 4: return [2 /*return*/];
            }
        });
    });
}
exports.createAccountInLedger = createAccountInLedger;
function createTrustline(accountKeypair) {
    return __awaiter(this, void 0, void 0, function () {
        var stellarServer, account, transaction, result, e_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    stellar_sdk_1.Network.useTestNetwork();
                    stellarServer = new stellar_sdk_1.Server('https://horizon-testnet.stellar.org');
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 4, , 5]);
                    return [4 /*yield*/, stellarServer.loadAccount(accountKeypair.publicKey())];
                case 2:
                    account = _a.sent();
                    transaction = new stellar_sdk_1.TransactionBuilder(account)
                        .addOperation(stellar_sdk_1.Operation.changeTrust({
                        asset: exports.asset
                    }))
                        .build();
                    transaction.sign(accountKeypair);
                    return [4 /*yield*/, stellarServer.submitTransaction(transaction)];
                case 3:
                    result = _a.sent();
                    console.log('trustline created from  account to issuer and signers updated', result);
                    return [2 /*return*/, result];
                case 4:
                    e_2 = _a.sent();
                    console.log('create trustline failed.', e_2.message);
                    return [3 /*break*/, 5];
                case 5: return [2 /*return*/];
            }
        });
    });
}
exports.createTrustline = createTrustline;
function allowTrust(trustor) {
    return __awaiter(this, void 0, void 0, function () {
        var stellarServer, issuingKeys, issuingAccount, transaction, result, e_3;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    stellar_sdk_1.Network.useTestNetwork();
                    stellarServer = new stellar_sdk_1.Server('https://horizon-testnet.stellar.org');
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 4, , 5]);
                    issuingKeys = stellar_sdk_1.Keypair.fromSecret(secretKey);
                    return [4 /*yield*/, stellarServer.loadAccount(issuingKeys.publicKey())];
                case 2:
                    issuingAccount = _a.sent();
                    transaction = new stellar_sdk_1.TransactionBuilder(issuingAccount)
                        .addOperation(stellar_sdk_1.Operation.allowTrust({
                        trustor: trustor,
                        assetCode: exports.asset.code,
                        authorize: true
                    }))
                        .build();
                    transaction.sign(issuingKeys);
                    return [4 /*yield*/, stellarServer.submitTransaction(transaction)];
                case 3:
                    result = _a.sent();
                    console.log('trust allowed');
                    return [2 /*return*/, result];
                case 4:
                    e_3 = _a.sent();
                    console.log('allow trust failed', e_3);
                    return [3 /*break*/, 5];
                case 5: return [2 /*return*/];
            }
        });
    });
}
exports.allowTrust = allowTrust;
function payment(signerKeys, destination, amount) {
    return __awaiter(this, void 0, void 0, function () {
        var stellarServer, account, transaction, result, e_4;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    stellar_sdk_1.Network.useTestNetwork();
                    stellarServer = new stellar_sdk_1.Server('https://horizon-testnet.stellar.org');
                    return [4 /*yield*/, stellarServer.loadAccount(signerKeys.publicKey())];
                case 1:
                    account = _a.sent();
                    transaction = new stellar_sdk_1.TransactionBuilder(account)
                        .addOperation(stellar_sdk_1.Operation.payment({
                        destination: destination,
                        asset: exports.asset,
                        amount: amount
                    }))
                        .build();
                    transaction.sign(signerKeys);
                    console.log("sending " + amount + " from " + signerKeys.publicKey() + " to " + destination + " ");
                    _a.label = 2;
                case 2:
                    _a.trys.push([2, 4, , 5]);
                    return [4 /*yield*/, stellarServer.submitTransaction(transaction)];
                case 3:
                    result = _a.sent();
                    return [2 /*return*/, result];
                case 4:
                    e_4 = _a.sent();
                    console.log("failure " + e_4.message);
                    throw e_4;
                case 5: return [2 /*return*/];
            }
        });
    });
}
exports.payment = payment;
//# sourceMappingURL=utils.js.map